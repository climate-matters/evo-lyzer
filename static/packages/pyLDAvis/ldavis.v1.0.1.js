...
var LDAvis = function (to_select, data_or_file_name) {
    ...
    var barFreqLegendID = visID + "-bar-freq-legend";
    ...
    function visualize(data) {
        ...
        self.set_topic = function (topic_id) {
            topic_off(document.getElementById(topicID + vis_state.topic));
            if (!isNaN(topic_id) && topic_id > 0) {
                let topic = Math.min(K, Math.max(1, topic_id));
                topic_on(document.getElementById(topicID + topic));
                vis_state.topic = topic;
                state_save(true);
                document.getElementById(topicID).value = self.vis_state.topic;
            }
        }
        ...
        var chartLegend = d3.select("#" + barFreqsID).append('g')
            .attr("transform", "translate(0," + +(mdsheight) + ")")
            .attr("id", barFreqLegendID);
        // bar chart legend/guide:
        d3.select("#" + barFreqLegendID).append("rect")
            .attr("x", 0)
            .attr("y", 10) // mdsheight +
            ...
            .style("fill", color1)
            .attr("opacity", 0.4);
        d3.select("#" + barFreqLegendID).append("text")
            .attr("x", barguide.width + 5)
            .attr("y", 10 + barguide.height / 2) // mdsheight +
            .style("dominant-baseline", "middle")
            .text("Overall term frequency");

        d3.select("#" + barFreqLegendID).append("rect")
            .attr("x", 0)
            .attr("y", 10 + barguide.height + 5)
            .attr("height", barguide.height)
        ...
        d3.select("#" + barFreqLegendID)
            ...
            .append("text")
            .attr("x", 0)
            .attr("y", 10 + (8 / 2) * barguide.height + 5)
            .style("dominant-baseline", "middle")
        ...
        function init_forms(topicID, lambdaID, visID) {
            ...
            inputDiv.setAttribute("style", "width: 100%"); // to match the width of the main svg element
            ...
            lambdaDiv.setAttribute("style", "display: flex; width: 100%; " +
                "background-color: #e8e8e8; align-items: center; flex-direction: column;");
            // padding: 5px; display: inline-block; height: 50px; width: 100%; float: right; margin-right: 30px"
            inputDiv.appendChild(lambdaDiv);

            var lambdaZero = document.createElement("div");
            lambdaZero.setAttribute("style", "font-family: sans-serif;");
            ...
            sliderDiv.setAttribute("style", "width: 250px;");
            ...
            lambdaLabel.setAttribute("style", "font-family: sans-serif; font-size: 14px;");
